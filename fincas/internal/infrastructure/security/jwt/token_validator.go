...
// ValidarAccessToken valida un access token JWT y retorna (usuarioID, sesionID, issuer, error).
// Valida: firma, expiración, issuer (si está configurado) y tipo (debe ser "access").
func (v *TokenValidator) ValidarAccessToken(tokenString string) (usuarioID, sesionID, issuer string, err error) {
	...
	return claims.Subject, claims.SesionID, claims.Issuer, nil
}
