...
**Alcance:**
- Comando ejecutable via `go run ./cmd/create-initial-sysadmin` o binario compilado
...
- **REQ-008**: El comando DEBE existir como un package `cmd/create-initial-sysadmin/` con su propio `main.go`.
...
```
Usage: go run ./cmd/create-initial-sysadmin [flags]
...
go run ./cmd/create-initial-sysadmin

# Modo full (todos los parámetros)
go run ./cmd/create-initial-sysadmin --full \
  --nombre "Admin" \
  --apellido "Sistema" \
...
1. **VC-001**: Existe el directorio `cmd/create-initial-sysadmin/` con un `main.go` compilable.
2. **VC-002**: `go run ./cmd/create-initial-sysadmin` se ejecuta sin errores y produce la salida esperada.
